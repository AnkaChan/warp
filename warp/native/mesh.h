...
CUDA_CALLABLE inline bool mesh_query_next_impl(mesh_query_aabb_t& query, int& index)
{
    ...
    // packed leaf currently being enumerated, or processes the node carried
    // over in registers / popped from the shared stack. Children are tested
    // at the parent; the near child rides in registers and far children go on
    // the stack as tagged payload pairs while the tree depth proves they fit
    // (see bvh_query_pair_limit), so pops need no memory access.
    for (;;) {
        ...
        if (bvh_query_node_is_leaf(node)) {
            ...
            // is the leaf node's AABB, which already passed its node test
            if (end - start == 1) {
